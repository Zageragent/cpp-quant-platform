...
Result<int> Config::get_int(std::string_view key) const { auto v=get_string(key); if(!v) return Err<int>(v.error()); try{ std::size_t idx=0; auto text=v.value(); int out=std::stoi(text,&idx); if(idx!=text.size()) return Err<int>(Error{ErrorCode::ConfigurationError,"trailing characters in integer",std::string(key)}); return Ok(out);}catch(const std::exception& e){return Err<int>(Error{ErrorCode::ConfigurationError,e.what(),std::string(key)});} }
Result<double> Config::get_double(std::string_view key) const { auto v=get_string(key); if(!v) return Err<double>(v.error()); try{ std::size_t idx=0; auto text=v.value(); double out=std::stod(text,&idx); if(idx!=text.size()) return Err<double>(Error{ErrorCode::ConfigurationError,"trailing characters in number",std::string(key)}); return Ok(out);}catch(const std::exception& e){return Err<double>(Error{ErrorCode::ConfigurationError,e.what(),std::string(key)});} }
