...
int main(){
  ...
  std::ofstream cfg_file("/tmp/qp_config.json"); cfg_file << "{\"seed\":42,\"threshold\":0.01,\"enabled\":true,\"run.id\":\"unit_\\\"run\\nwith_escape\",\"windows.path\":\"C:\\\\tmp\\\\bars.csv\"}"; cfg_file.close(); auto cfg=load_flat_json_config("/tmp/qp_config.json"); CHECK(cfg.ok()); CHECK(cfg.value().get_int("seed").value()==42); CHECK(near(cfg.value().get_double("threshold").value(),0.01)); CHECK(cfg.value().get_bool("enabled").value()); CHECK(cfg.value().get_string("run.id").value()=="unit_\"run\nwith_escape"); CHECK(cfg.value().get_string("windows.path").value()=="C:\\tmp\\bars.csv"); Config cfg2; cfg2.set("seed","43"); cfg2.set("threshold","0.01"); cfg2.set("enabled","true"); CHECK(stable_config_fingerprint(cfg.value()) != stable_config_fingerprint(cfg2));
